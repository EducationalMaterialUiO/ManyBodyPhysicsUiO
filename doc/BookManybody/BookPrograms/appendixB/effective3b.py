"""
Effective three-body interactions from exact diagonalisation.

Companion code to Appendix B of *Quantum mechanics for Many-particle Systems*
and to the FYS4480 lecture notes.

The question this program answers is the following.  Suppose we can solve the
few-body problem exactly in a large single-particle basis, but we want to work
in a small *model space* P spanned by the lowest single-particle orbitals.  An
effective two-body interaction that reproduces the exact two-body spectrum in
P can be constructed with the Lee-Suzuki-Okubo similarity transformation.  The
same construction for three particles yields an effective three-body
Hamiltonian, and the part of it that is *not* accounted for by the embedded
one- and two-body effective operators is an induced effective three-body
interaction,

    V3_eff = H3_eff - sum_i t_i - sum_{i<j} V2_eff(ij).

The program implements this cluster hierarchy on a toy model: spinless
fermions in a one-dimensional harmonic oscillator interacting through a
Gaussian two-body potential.  The ingredients are
  ...
  * ``DeterminantBasis``      -- Slater determinants as ordered tuples of
    occupied orbitals (and as bit strings);
  * ``KBodyOperator``         -- a k-body operator stored through its matrix
    in the k-particle determinant basis, with a routine that embeds it in the
    N-particle space (this is the operator identity
    O = sum_{S,P} <S|O|P> a+_S a_P, see the notebook);
...
Everything runs on numpy and scipy.linalg in a few seconds.
...
class HarmonicOscillator1D:
    ...
    def one_body(self, gap=0.0, n_P=None):
        ...
        exactly degenerate -- the intruder-state problem discussed in the
        notebook.
        """
...
# ---------------------------------------------------------------------------
def _annihilate(bits, p):
    """a_p acting on a bit string; returns (sign, new bits), sign 0 if empty."""
    if not (bits >> p) & 1:
        return 0, bits
    sign = -1 if popcount(bits & ((1 << p) - 1)) % 2 else 1
    return sign, bits & ~(1 << p)


def _create(bits, p):
    """a+_p acting on a bit string; returns (sign, new bits), sign 0 if occupied."""
    if (bits >> p) & 1:
        return 0, bits
    sign = -1 if popcount(bits & ((1 << p) - 1)) % 2 else 1
    return sign, bits | (1 << p)
...
class KBodyOperator:
    ...
    extracted in the notebook is k = 3.
    """
